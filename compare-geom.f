...
c     ****          are stored and subsequent ifref=0 calcs use the difference
c     ****	    to these coords and project this diff onto the ref nc's
c

      parameter (m=300, m3=3*m, mr=m*20)
...
c     **** used to identify zero freq modes ****
      ...
      open (5,file='compare-geom.dat',status='old')
      open (1,file='supplem.dat',status='unknown')
      open (2,file='supplem.ind',status='unknown')
      open (3,file='supplem.freq',status='unknown')
      open (4,file='supplem.rotn',status='unknown')
      open (7,file='supplem.geom',status='unknown')
      open (8,file='supplem.imag',status='unknown')
      open (16,file='compare-geom.out',status='unknown')
...
C      call inertm (x0,c0,n,m3,n3,nat1,atmas)
...
C      call wsupp (mname0,n,m3,x0,nat1,symm0,eval0,isir,fir,zptl0,c0,
C     $      ener,ptgrp,iwk,f0,coord, ni1,z1,intdf1,iztype,scale)
...
C      call inertm (x2,c2,nkeep,m3,n23,nat2,atmas)

c     **** swap atoms, change axis dirn to allign old and new ****

      i= iforder
      if (ncalc.le.2 .and. iforder.eq.1) i= 2
C      call align (x1,x2,wk,nat1,nat2,n,nkeep,ord,wk3,c2,m3,n23,i)
...
C      call wsupp (mname2,n,m3,x2,nat2,symm2,eval2,isir,fir,zptl2,c2,
C     $      ener,ptgrp,iwk,f0,coord, ni2,z2,intdf2,iztype,scale)
...
c	**** test int coords are the same ****
	...
	do i= 1,ni
	  ...
	  write (6,*) i,iztype(i),(intdf1(j,i),j=1,4),z1(i),z2(i)
	end do
	if (ni1.ne.ni2) then
...
c	**** calculate change in curvilinear internal coords  ****
	...
	do i= 1,ni
     ...
     $	      iztype(i).eq.2 .and. abs(zd(i)).gt.0.03 .or.
     $	     abs(zd(i)).gt.0.1) then
	    ...
	    write (16,'(a,i5,i2,4(3x,a5),2f8.3,f10.3)')
     $	         ' zd',i,iztype(i),intlb,z1(i),z2(i),zd(i)*scale(i)
	  end if
	end do
...
C	    stop 'asym displ'
	  ...
	  if (is.eq.1) then
	    i1= i1 + 1
	    write (out1(i1),'(i2,1x,a3,i5,f8.4,f7.3)')
...
C	    stop 'asym displ'
	  ...
	  if (is.eq.1) then
	    i2= i2 + 1
	    write (out2(i2),'(i2,1x,a3,i5,f8.4,f7.3)')
...
710   format (/'   Proj of coord change from ',a,
	...
	if (n23.gt.0 .and. i.le.i2) then
	  write (1,'(1x,a30,10x,a30)') out1(i),out2(i)
	else
	  write (1,'(1x,a30)') out1(i)
	end if
      end do
      if (n23.gt.0) then
